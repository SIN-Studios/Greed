...
enum states {idleing, attacking, hurting, charging, rolling}
...
func _process(_delta: float) -> void:
	if state == states.idleing:
		...
		enemy_function.hurting(self, enemy_hurt_damage, player)
		state = previous_state
	elif state == states.charging:
		enemy_function.charging(self, player)
		state = states.idleing
	#elif state == states.rolling(self, player):
	#	enemy_function.rolling
